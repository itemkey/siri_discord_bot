...
REMOVED_PUBLIC_PRIVATE_KEYS = ("personal", "specials")
...
class Bunker(commands.Cog):
    ...
    async def debug_command(self, interaction: discord.Interaction) -> None:
        if not await self._is_bunker_operator(interaction):
            ...
            return
        await self.open_game_panel(interaction, status="Debug-режим: доступны тест-боты, форс-старт и смена фазы только для админ-игры.")
        ...
        *,
        ...
        if screen == "packs":
            ...
            if status:
                embed.set_footer(text=status)
            return embed, back_view
        if screen in {"card", "reveal"}:
            if player is None or player.card is None:
                return _status_embed("Карточка появится после старта игры."), back_view
            status = "Твоя карточка. Статусы показывают, что уже знают остальные участники."
            view: discord.ui.View = back_view
            if game.state != GameState.REVEAL_PHASE:
                return _personal_card_embed(player, status=status), view
            ...
            if current_player is None or current_player.user_id != interaction.user.id:
                ...
                status = f"Сейчас ход {name}. Твоя карточка ниже."
                return _personal_card_embed(player, status=status), view
            if player.is_eliminated:
                status = "Ты выгнан из бункера, но твоя карточка остается видна тебе."
                return _personal_card_embed(player, status=status), view
            stats = selectable_reveal_stats(player)
            remaining = reveal_turn_remaining(player, game.settings, game.reveals_done_this_turn)
            status = _reveal_choice_status(game, player)
            if stats and remaining > 0:
                view = BunkerRevealView(self, game, interaction.user.id, player)
            elif not stats:
                status = "Ты уже раскрыл все характеристики. Твоя карточка ниже."
            else:
                status = "Лимит раскрытий за этот ход уже исчерпан. Твоя карточка ниже."
            return _personal_card_embed(player, status=status), view
        if screen == "vote":
    ...
    async def build_bunker(self, interaction: discord.Interaction, *, is_admin_game: bool = False) -> None:
        ...
        if not isinstance(setup_channel, discord.TextChannel):
            await build_status("Setup-канал больше недоступен.")
            return
        ...
        try:
            ...
            if is_admin_game and operator_role is None:
                await build_status("Сначала назначь operator-role через /opbunker role.")
                return
            if is_admin_game and not await self._is_bunker_operator(interaction):
                await build_status("Admin-test комнату может создать только operator-role.")
                return
        ...
        await build_status(f"Бункер построен: {text_channel.mention}. {suffix}", voice_channel=voice_channel)
    ...
    async def reveal_selected_stat(self, interaction: discord.Interaction, game_id: int, user_id: int, stat: str) -> None:
        ...
        remaining = reveal_turn_remaining(player, game.settings, game.reveals_done_this_turn)
        if remaining <= 0:
            await interaction.response.edit_message(embed=_status_embed("Лимит раскрытий за этот ход уже исчерпан."), view=BunkerPanelBackView(self, game_id))
            return
    ...
    async def public_reveal_selected_stat(self, interaction: discord.Interaction, stat: str) -> None:
        await interaction.response.send_message(
            "Раскрытие характеристик переехало в личную панель: нажми `Панель` -> `Личная информация`.",
            ephemeral=True,
        )

    async def public_show_current_reveal_card(self, interaction: discord.Interaction) -> None:
        await interaction.response.send_message(
            "Полная карточка теперь открывается через личную панель: нажми `Панель` -> `Личная информация`.",
            ephemeral=True,
        )
    ...
    async def _advance_reveal_progress(
        ...
        if not player_completed_reveal_turn(revealed_player, game.settings, reveals_done_after):
            await self.repository.set_reveal_progress(
                game.id,
                current_turn_index=current_index,
                reveals_done_this_turn=reveals_done_after,
            )
            return

        current_index += 1
        while current_index < len(ordered) and player_completed_reveal_turn(ordered[current_index], game.settings, 0):
            current_index += 1
    ...
    async def use_special_action(
        ...
        if game is None or player is None or player.card is None:
            ...
            return
        if not await self._can_act_as_public_player(interaction, game, player):
        ...
        if not ability.revealed:
            ...
            await self.refresh_game_message(game.id)
            fresh = await self.repository.get_game(game.id) or game
            await self.update_current_game_panel(interaction, fresh, screen="action", status="Спец. возможность раскрыта. Теперь ее можно использовать.")
            return
    ...
    async def public_use_special_ability(self, interaction: discord.Interaction, ability_index: int) -> None:
        await interaction.response.send_message(
            "Спец. возможности переехали в личную панель: нажми `Панель` -> `Спец возможности`.",
            ephemeral=True,
        )

    async def _resolve_public_message_game(
        self,
        interaction: discord.Interaction,
        *,
        expected_key: str,
    ) -> tuple[BunkerGame, str] | None:
    ...
    async def _can_act_as_public_player(
        self,
        interaction: discord.Interaction,
        game: BunkerGame,
        player: BunkerPlayer,
        ...
        if interaction.user.id == player.user_id:
            return True
        is_admin_test = game.room_kind == RoomKind.ADMIN_TEST or game.is_admin_game
        return bool(player.is_fake and is_admin_test and await self._is_bunker_operator(interaction))
    ...
    async def refresh_game_message(self, game_id: int) -> None:
        ...
        await message.edit(embed=embed, attachments=[], view=_leader_view_for_game(self, game))
    ...
    async def _send_board_message(self, channel: discord.TextChannel, game: BunkerGame, players: list[BunkerPlayer]) -> discord.Message:
        embed = _game_embed(game, players)
        return await channel.send(embed=embed, view=_leader_view_for_game(self, game))
    ...
    async def _sync_public_game_messages(
        ...
        await self._delete_removed_public_messages(game, channel, REMOVED_PUBLIC_PRIVATE_KEYS)
        ...
        for key, embed in payloads:
            ...
            collapsible = key in {"cataclysm", "bunker", "players", "abilities"}
            payload = _collapsed_public_embed(key) if collapsed and collapsible else embed
            view: discord.ui.View | None
            if collapsible:
    ...
    async def _delete_removed_public_messages(
        self,
        game: BunkerGame,
        channel: discord.TextChannel,
        keys: tuple[str, ...],
        ...
        for key in keys:
            ...
            try:
                ...
                LOGGER.info("Bunker removed public message %s/%s is missing.", game.id, key, exc_info=True)
            remover = getattr(self.repository, "remove_public_message_id", None)
            if remover is not None:
                await remover(game.id, key)
            game.public_message_ids.pop(key, None)
...
class BunkerPublicRevealView(SafeView):
    def __init__(
        ...
        cog: Bunker,
        game: BunkerGame | None = None,
        player: BunkerPlayer | None = None,
    ) -> None:
        super().__init__(timeout=None)
        self.cog = cog
        stats = REVEALABLE_STATS
        revealed_stats = set(player.revealed_stats) if player is not None else set()
        turn_limit_reached = (
            game is not None
            and player is not None
            and game.state == GameState.REVEAL_PHASE
            and reveal_turn_remaining(player, game.settings, game.reveals_done_this_turn) <= 0
        )
        for index, stat in enumerate(stats):
            ...
            async def callback(interaction: discord.Interaction, selected_stat: str = stat) -> None:
                await self.cog.public_reveal_selected_stat(interaction, selected_stat)

            button.callback = callback
            self.add_item(button)

        self_button = discord.ui.Button(
            label="Показать мои данные лично",
            style=discord.ButtonStyle.primary,
    ...
    def __init__(
        ...
        for index in range(2):
            ...
            button = discord.ui.Button(
                label=label,
                style=discord.ButtonStyle.secondary,
                custom_id=f"{PUBLIC_ABILITY_PREFIX}{index}",
                disabled=disabled,
                row=0,
            )

            async def callback(interaction: discord.Interaction, ability_index: int = index) -> None:
                await self.cog.public_use_special_ability(interaction, ability_index)
...
class BunkerPrivatePlayerPanelView(SafeView):
    ...
    def _build(self) -> None:
        ...
        if is_active_player and self.player.card is not None and not in_lobby:
            self._add_button("Личная информация", discord.ButtonStyle.primary, self._reveal, row=1)
...
class BunkerSettingsView(SafeView):
    def __init__(
        ...
        *,
        ...
        if self.screen == "settings_rules":
            self.add_item(BunkerRevealStatsPerTurnSelect(self, row=0))
            self.add_item(BunkerTimerSelect(self, row=1))
            self.add_item(BunkerMissingVotePolicySelect(self, row=2))
            self.add_item(BunkerRoundsSelect(self, row=3))
            self._add_button("Основные", discord.ButtonStyle.secondary, self._main_settings, row=4)
        else:
            self.add_item(BunkerRoomKindSelect(self, row=0))
            self.add_item(BunkerVisibilitySelect(self, row=1))
            self.add_item(BunkerModeSelect(self, row=2))
            self.add_item(BunkerSlotsSelect(self, row=3))
            self._add_button("Правила/таймеры", discord.ButtonStyle.secondary, self._rules_settings, row=4)
        self._add_button("Контент", discord.ButtonStyle.secondary, self.open_content, row=4)
        self._add_button("Как играть", discord.ButtonStyle.secondary, self.open_rules, row=4)
    ...
    async def _main_settings(self, interaction: discord.Interaction) -> None:
        await self.open_settings_section(interaction, "settings")

    async def _rules_settings(self, interaction: discord.Interaction) -> None:
        await self.open_settings_section(interaction, "settings_rules")
    ...
    async def back(self, interaction: discord.Interaction) -> None:
        if self.game_id is not None:
            ...
            return
        await interaction.response.edit_message(
            embed=_status_embed("Настройки закрыты. Чтобы снова открыть их, нажми 'Настроить бункер' в панели комнаты."),
            view=None,
        )
...
class BunkerRoomKindSelect(discord.ui.Select):
    def __init__(self, owner: BunkerSettingsView, *, row: int) -> None:
        ...
        options = [
            discord.SelectOption(
                ...
                value=RoomKind.RANKED.value,
                description="Единые правила, без fake-ботов и debug.",
                default=owner.settings.room_kind != RoomKind.ADMIN_TEST,
            )
        ]
        if owner.is_operator:
            options.append(
                discord.SelectOption(
                    label="Админ-игра",
                    value=RoomKind.ADMIN_TEST.value,
                    description="Скрытая тестовая комната с fake-ботами.",
                    default=owner.settings.room_kind == RoomKind.ADMIN_TEST,
                )
            )
        super().__init__(
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        room_kind = RoomKind.ADMIN_TEST if self.values[0] == RoomKind.ADMIN_TEST.value else RoomKind.RANKED
...
class BunkerRevealStatsPerTurnSelect(discord.ui.Select):
    def __init__(self, owner: BunkerSettingsView, *, row: int) -> None:
        self.owner = owner
        values = (1, 2, 3)
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        await self.owner.save(interaction, replace(self.owner.settings, reveal_stats_per_turn=int(self.values[0])))
...
class BunkerActionView(SafeView):
    def __init__(self, cog: Bunker, game_id: int, player: BunkerPlayer) -> None:
        ...
        for index, ability in enumerate((player.card.special_abilities if player.card else ())[:2]):
            ...
            if ability.used:
                label = f"Использована: {ability.name}"[:80]
            elif ability.blocked or player.is_eliminated:
                label = f"Недоступна: {ability.name}"[:80]
            elif ability.revealed:
                label = f"Использовать: {ability.name}"[:80]
            else:
                label = f"Раскрыть: {ability.name}"[:80]
            if disabled:
                style = discord.ButtonStyle.secondary
            elif ability.revealed:
                style = discord.ButtonStyle.primary
...
def _reveal_choice_status(game: BunkerGame, player: BunkerPlayer) -> str:
    remaining = reveal_turn_remaining(player, game.settings, game.reveals_done_this_turn)
    limit = reveal_turn_limit(game.settings)
    opened = max(0, min(limit, limit - remaining))
    return f"Можно открыть: {opened}/{limit}. Выбери любую скрытую характеристику."


def _public_personal_embed(game: BunkerGame, players: list[BunkerPlayer]) -> discord.Embed:
    player = _current_ordered_player(game, players, kind="reveal") if game.state == GameState.REVEAL_PHASE else None
    ...
    if player is None:
        ...
        return embed
    privacy_hint = "Это публичная версия: скрытые характеристики закрыты для остальных. Текущий игрок видит полную карточку через кнопку `Показать мои данные лично`."
    embed.description = (
...
def _public_specials_embed(game: BunkerGame, players: list[BunkerPlayer]) -> discord.Embed:
    player = _current_public_action_player(game, players)
    ...
    for index, player in enumerate(players[:16], start=1):
        if player.card is None:
            ...
            for ability in player.card.special_abilities[:2]:
                statuses.append(_ability_status(player, ability))
...
def _player_status_label(player: BunkerPlayer) -> str:
...
def _room_kind_label(room_kind: RoomKind) -> str:
    return "Админ-игра" if room_kind == RoomKind.ADMIN_TEST else "Рейтинговая игра"
...
def _reveal_prompt(game: BunkerGame, players: list[BunkerPlayer]) -> str:
    ...
    return (
        f"Раунд {game.round_number}. Ход {format_player_name(player)}. "
        "Раскрытие делается через личную панель: `Панель` -> `Личная информация`."
    )
...
def _game_embed(game: BunkerGame, players: list[BunkerPlayer]) -> discord.Embed:
    ...
    if game.state == GameState.LOBBY:
        ...
        rows = [
            ...
            ("Режим", _room_kind_label(game.room_kind)),
    ...
    embed.add_field(name="Режим", value=_room_kind_label(game.room_kind), inline=True)
    prompt = {
...
def _current_public_action_player(game: BunkerGame, players: list[BunkerPlayer]) -> BunkerPlayer | None:
    if game.state == GameState.REVEAL_PHASE:
        return _current_ordered_player(game, players, kind="reveal")
    if game.state == GameState.SPEECH_PHASE:
        return _current_ordered_player(game, players, kind="speech")
    return None
